...
STRATEGY_BASE_PROFILE="${STRATEGY_BASE_PROFILE:-hi_win}"
STRATEGY_BASE_PROFILE_BY_SCOPE="${STRATEGY_BASE_PROFILE_BY_SCOPE:-BTCUSDT:5m=btc5m_balance_2026_03_06,SOLUSDT:5m=sol5m_sharp_2026_03_07}"
...
MARKET_CANDIDATE_POOL_SIZE="${MARKET_CANDIDATE_POOL_SIZE:-3}"
DISCOVERY_PREWARM_LEAD_MS="${DISCOVERY_PREWARM_LEAD_MS:-300000}"
DISCOVERY_PREWARM_REFRESH_SEC="${DISCOVERY_PREWARM_REFRESH_SEC:-2}"
DISCOVERY_MAX_PAST_MS="${DISCOVERY_MAX_PAST_MS:-240000}"
...
if [ -f "$ENV_FILE" ]; then
  # Strategy profile selection is operator-managed in .env or left unset for code defaults.
...
Environment=POLYEDGE_DISCOVERY_MAX_FUTURE_MS_5M=1800000
Environment=POLYEDGE_DISCOVERY_MAX_FUTURE_MS_15M=5400000
...
Environment=POLYEDGE_DISCOVERY_MAX_PAST_MS=$DISCOVERY_MAX_PAST_MS
...
Environment=FORGE_MARKET_SELECTION_FALLBACK_PREWARM_MS=$MARKET_SELECTION_FALLBACK_PREWARM_MS
Environment=FORGE_MARKET_CANDIDATE_POOL_SIZE=$MARKET_CANDIDATE_POOL_SIZE
Environment=FORGE_DISCOVERY_PREWARM_LEAD_MS=$DISCOVERY_PREWARM_LEAD_MS
Environment=FORGE_DISCOVERY_PREWARM_REFRESH_SEC=$DISCOVERY_PREWARM_REFRESH_SEC
Environment=FORGE_ROUND_MIN_COVERAGE_RATIO=$ROUND_MIN_COVERAGE_RATIO
Environment=FORGE_ROUND_MIN_SAMPLE_RATIO=$ROUND_MIN_SAMPLE_RATIO
...
Environment=FORGE_FEV1_RUNTIME_LOOP_MS=180
...
Environment=FORGE_STRATEGY_MAX_POINTS_FULL=220000
