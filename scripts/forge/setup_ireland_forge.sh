...
STRATEGY_BASE_PROFILE_BY_SCOPE="${STRATEGY_BASE_PROFILE_BY_SCOPE:-SOLUSDT:5m=hi_win}"
...
if [ -f "$ENV_FILE" ]; then
  ...
  upsert_env "FORGE_STRATEGY_BASE_PROFILE_BY_SCOPE" "$STRATEGY_BASE_PROFILE_BY_SCOPE" "$ENV_FILE"
...
Environment=FORGE_TOKYO_RELAY_QUEUE_CAP=65536
Environment=FORGE_CHAINLINK_QUEUE_CAP=16384
Environment=FORGE_BOOK_QUEUE_CAP=32768
