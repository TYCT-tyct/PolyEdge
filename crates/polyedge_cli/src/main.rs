...
fn run_deploy_ireland(base: RemoteBase) -> Result<()> {
    ...
    run_ssh(&base, &target, &format!("set -euo pipefail; cd {}; git fetch origin; git checkout {}; git pull --ff-only origin {}; cargo build -p app_runner -p polyedge_cli --release", base.repo, base.branch, base.branch))?;
